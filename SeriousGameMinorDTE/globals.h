...
std::string explainations[AMOUNT_OF_SUBJECTS][MAX_QUESTIONS] = { {"LiDAR data contains large amounts of numbers and therefore needs to be able to handle large amounts of data. Making it easier to produce new queries also makes this handling easier. However being widely used and easy to understand is not necessary, but could improve readability and the depth of the modelling tool.",
																...
																{"Rain droplets may reflect the LiDAR waves and cause the reading to be a shorter distance than what you actually want to measure (Goodin, Carruth, Doude, & Hudson, 2019).", 
																"When a rolling shutter camera tries to capture an object that is moving at very high speeds, the object may turn out malformed in the end result (Hokuyo automatic co., 2012).", 
																"( Jia & Wang 2019)", 
																"Harsh sunlight may cause more reflection than normal on already reflective surfaces. Rain may cause distances to be measured incorrectly. Fog may impede the waves of the LiDAR camera ( Hokuyo automatic co., 2012).", 
																"The doppler effect does not affect measurements regarding stationary objects, but rather objects that are approaching or receding from the camera (Goodin, Carruth, Doude, & Hudson, 2019).", 
																"Noisy data is data that does not provide useful information to a research (Rouse, 2010).", 
																"Removing noise with an algorithmic approach means to use algorithms to remove data points based on classifiers for what is and isn\'t data noise. Removing data noise on a data level means to apply filters to a dataset to limit the insertion of data that does not fit the filter\'s criteria (Sáez & Galar & Luengo & Herrera, 2015).", 
																"Outliers may be considered data noise in a certain context, but they are actually two separate things. noise can be defined as mislabeled examples or errors in the values of attributes, whereas outliers are defined as abnormalities or anomalies (Rouse, 2010).", 
																"INFFC is a noise removal method that is based on a fusion of multiple classifiers to detect noise. The use of multiple classifiers in an iterative way makes this a very accurate way of removing data noise, but also a very time consuming one (Sáez & Galar & Luengo & Herrera, 2015).", 
																"While atmospheric noise can be a source of data noise in certain specific contexts it isn\'t applicable to our context (Hokuyo automatic co., 2012)."},
